...
/**
 * Base IndexInput implementation that uses an array of ByteBuffers to represent
 * a file.
 * <p>
 * Because Java's ByteBuffer uses an int to address the values, it's necessary
 * to access a file greater Integer.MAX_VALUE in size using multiple byte
 * buffers.
 * <p>
 * For efficiency, this class requires that the buffers are a power-of-two
 * (<code>chunkSizePower</code>).
 */
public abstract class MyByteBufferIndexInput extends IndexInput implements RandomAccessInput {
    ...
    protected final MyByteBufferGuard guard;
    ...
    public static MyByteBufferIndexInput newInstance(String resourceDescription, ByteBuffer[] buffers, long length, int chunkSizePower, MyByteBufferGuard guard) {
        if (buffers.length == 1) {
    ...
    public static MyByteBufferIndexInput newInstance(DirConfig config, String name) throws IOException {
        ...
        MyByteBufferGuard guard = new MyByteBufferGuard(name, null);
        
        return newInstance(name, buffs, len, DEFAULT_MAX_CHUNK_SIZE, guard);
    }

    public MyByteBufferIndexInput(String resourceDescription, ByteBuffer[] buffers, long length, int chunkSizePower, MyByteBufferGuard guard) {
    ...
    @Override
    public final MyByteBufferIndexInput clone() {
        final MyByteBufferIndexInput clone = buildSlice((String) null, 0L, this.length);
        try {
     ...
     */
    @Override
    public final MyByteBufferIndexInput slice(String sliceDescription, long offset, long length) {
     ...
     * subclasses). *
     */
    protected MyByteBufferIndexInput buildSlice(String sliceDescription, long offset, long length) {
        if (buffers == null) {
            throw new AlreadyClosedException("Already closed: " + this);
        }
        ...
        final MyByteBufferIndexInput clone = newCloneInstance(getFullSliceDescription(sliceDescription), newBuffers, ofs, length);
        clone.isClone = true;
    ...
    protected MyByteBufferIndexInput newCloneInstance(String newResourceDescription, ByteBuffer[] newBuffers, int offset, long length) {
    ...
    /**
     * Returns a sliced view from a set of already-existing buffers: the last
     * buffer's limit() will be correct, but you must deal with offset
     * separately (the first buffer will not be adjusted)
     */
    ...
    /**
     * Optimization of MyByteBufferIndexInput for when there is only one buffer
     */
    public static final class SingleBufferImpl extends MyByteBufferIndexInput {

        SingleBufferImpl(String resourceDescription, ByteBuffer buffer, long length, int chunkSizePower, MyByteBufferGuard guard) {
    ...
    /**
     * This class adds offset support to MyByteBufferIndexInput, which is needed
     * for slices.
     */
    public static final class MultiBufferImpl extends MyByteBufferIndexInput {

        private final int offset;

        MultiBufferImpl(String resourceDescription, ByteBuffer[] buffers, int offset, long length, int chunkSizePower,
                MyByteBufferGuard guard) {
        ...
        @Override
        protected MyByteBufferIndexInput buildSlice(String sliceDescription, long ofs, long length) {
