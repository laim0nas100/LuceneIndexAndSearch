 ...
 */
public final class FlushByteBuffersDataInput extends DataInput implements Accountable, RandomAccessInput {
   ...
   */
  public FlushByteBuffersDataInput(List<ByteBuffer> buffers) {
  ...
  public FlushByteBuffersDataInput slice(long offset, long length) {
    ...
    return new FlushByteBuffersDataInput(sliceBufferList(Arrays.asList(this.blocks), offset, length));
  }
  ...
  private static List<ByteBuffer> sliceBufferList(List<ByteBuffer> buffers, long offset, long length) {
    ...
    if (buffers.size() == 1) {
      ...
      int blockBytes = FlushByteBuffersDataInput.determineBlockPage(buffers);
