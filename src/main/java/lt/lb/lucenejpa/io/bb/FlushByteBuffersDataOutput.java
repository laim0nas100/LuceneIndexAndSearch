 ...
 */
public final class FlushByteBuffersDataOutput extends DataOutput implements Accountable {
   ...
   */
  public FlushByteBuffersDataInput toDataInput() {
    return new FlushByteBuffersDataInput(toBufferList());
  }
  ...
  public void copyTo(DataOutput output) throws IOException {
    for (ByteBuffer bb : blocks) {
      if (bb.hasArray()) {
        ...
        bb.flip();
        output.copyBytes(new FlushByteBuffersDataInput(Collections.singletonList(bb)), bb.remaining());
      }
    }
  }
  ...
  /**
   * @return Returns a new {@link FlushByteBuffersDataOutput} with the {@link #reset()} capability. 
   */
  // TODO: perhaps we can move it out to an utility class (as a supplier of preconfigured instances?) 
  public static FlushByteBuffersDataOutput newResettableInstance() {
    FlushByteBuffersDataOutput.ByteBufferRecycler reuser = new FlushByteBuffersDataOutput.ByteBufferRecycler(
        FlushByteBuffersDataOutput.ALLOCATE_BB_ON_HEAP); 
    return new FlushByteBuffersDataOutput(
        FlushByteBuffersDataOutput.DEFAULT_MIN_BITS_PER_BLOCK,
        FlushByteBuffersDataOutput.DEFAULT_MAX_BITS_PER_BLOCK,
        reuser::allocate,
        reuser::reuse);
  }
  ...
  private void rewriteToBlockSize(int targetBlockBits) {
    ...
    FlushByteBuffersDataOutput cloned = new FlushByteBuffersDataOutput(targetBlockBits, targetBlockBits, blockAllocate, NO_REUSE);
    ByteBuffer block;
